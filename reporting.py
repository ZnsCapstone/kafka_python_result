...
def save_summary_report(all_results, current_round, final=False):
    ...
    lines.extend([
        ...
        f"- Storage backend: {cfg.storage_label()}",
        f"- Benchmark profile: {cfg.ACTIVE_PROFILE}",
        f"- Scenario group: {cfg.ACTIVE_SCENARIO_GROUP}",
        f"- Raw device: {cfg.RAW_ZNS_DEVICE if cfg.STORAGE_BACKEND == 'dm-zns' else cfg.FS_DEVICE}",
        ...
        for size in cfg.RECORD_SIZES:
            ...
            for scenario in cfg.SCENARIO_KEYS:
                ...
                lines.extend([
                    ...
                    f"    storage util / await: {safe_mean(metric('mapper_util_avg' if cfg.STORAGE_BACKEND == 'dm-zns' else 'raw_util_avg')):.2f} % / {safe_mean(metric('mapper_await_avg' if cfg.STORAGE_BACKEND == 'dm-zns' else 'raw_await_avg')):.2f} ms",
